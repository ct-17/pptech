...
class so_excel_report(models.Model):
    _name = "so.excel.template"
    sale_orde = fields.Many2one('sale.order', string="Sale Order")

    def print_xls_report(self, cr, uid, ids, context=None):
        data=self.read(cr,uid,ids)[0]
        return {'type': 'ir.actions.report.xml',
                'report_name':'dp_so_excel_template.so_excel_template.xlsx',
                'datas':data
        }

class so_excel_template(ReportXlsx):
    _name = 'report.dp_so_excel_template.so_excel_report'
    ...
    def _sale_order_report(self, data, objs):
        self.ws_stock_report = self.wb.add_worksheet('Draft Purchase Order')
        self.ws_stock_report.set_landscape()
        self.ws_stock_report.set_paper(self.a4_paper)
        self.ws_stock_report.fit_to_pages(1, 1)

        # SET REPORT TITLE
        self.ws_stock_report.merge_range('A1:F2', 'Draft Purchase Order', self.bold_center_header)
        ...
        if objs.parent_partner_id.exists():
            ...
            customer_name = objs['partner_id']['name'] or ''
            customer_code = objs['partner_id']['company_code'] or ''
            customer_street = objs['partner_id']['street'] or ''
            customer_street2 = objs['partner_id']['street2'] or ''
            customer_city = objs['partner_id']['city'] or ''
            customer_state_id_name = objs['partner_id']['state_id']['name'] or ''
            customer_zip = objs['partner_id']['zip'] or ''
            customer_country_id_name = objs['partner_id']['country_id']['name'] or ''
        ...
        self.ws_stock_report.write('E8', 'Order No', self.heading_bold_center)
        ...
        self.ws_stock_report.write('E11', 'Chandler PO No', self.heading_bold_center)
        self.ws_stock_report.write('F11', chandler_po_num, self.table_content_center)
        self.ws_stock_report.write('E12', 'Chandler Marking No', self.heading_bold_center)
        self.ws_stock_report.write('F12', chandler_marking_num or None, self.table_content_center)
        self.ws_stock_report.write('E13', 'ETA', self.heading_bold_center)
        self.ws_stock_report.write('F13', eta, self.table_content_center)
        self.ws_stock_report.write('E14', 'ETD', self.heading_bold_center)
        self.ws_stock_report.write('F14', etd, self.table_content_center)
        self.ws_stock_report.merge_range('E15:E16', 'Next Port of Call', self.heading_bold_center)
        self.ws_stock_report.merge_range('F15:F16', next_port, self.table_content_center)
        ...
        for items in objs['order_line']:
            ...
            self.ws_stock_report.write(row, 1,items['product_uom_qty'], self.data_bottomless_right)
            ...
            self.ws_stock_report.write(row, 4, items['tax_id']['display_name'], self.data_bottomless_right)
            self.ws_stock_report.write(row, 5, '{0:.2f}'.format(items['chandler_price_subtotal']), self.data_bottomless_right)
            row += 1
            counter += 1
        ...
        self.ws_stock_report.write(row + actual_inc, 5, '{0:.2f}'.format(objs['amount_total']), self.footer_data_top_border)
    ...
    def generate_xlsx_report(self, wb, data, objs):
        ...
        # report.name = "Order Entry (Excel) " + objs['name']
        self.wb = wb
        self._init_report_()
        self._sale_order_report(data, objs)
        return

so_excel_template('report.sale.order.xlsx', 'sale.order')
