...
class ProductPricelistImport(models.TransientModel):
    ...
    def operating_method_normal(self, time_start, lines, header, header_fields, reader):
        ...
        for line in reader:
            ...
            ## Import Script
            if line.get('productcode') and line.get('minquantity') and line.get(
                    'pricesurcharge') and line.get('discount') and line.get('sequence') \
                    and self.pricelist and self.pricelist_version:

                create_values = {
                    'sequence': line.get('sequence') or 1,
                    'price_version_id': self.pricelist_version.id,
                    'price_surcharge': str2float(
                        line.get('pricesurcharge'), self.decimal_separator
                    ),
                    'price_discount': str2float(
                        line.get('discount'), self.decimal_separator),
                    'base': self.base
                }

                # min_quantity if used
                if line.get('minquantity'):
                    create_values['min_quantity'] = str2float(
                        line.get('minquantity'), self.decimal_separator
                    )
                ...
                if self.productcode_options in ['product',
                    ...
                    if products and not supplierinfos:
                        ...
                        create_values['name'] = products.default_code
                ...
                raise UserError(
                        _("Wrong"),
                        _("Error while processing import "))
    ...
    def search_existing_item(self, line):
        ...
        if line.get('productcode') and \
            ...
            # if minquantity is used remove only with the minquantity
            if line.get('minquantity'):
