openerp.report_xlsx =  function(instance) {
    // var trigger_download = instance.report.trigger_download();
    ...
    instance.web.ActionManager = instance.web.ActionManager.extend({
        ir_actions_report_xml: function(action, options) {
            ...
            // QWeb reports
            if ('report_type' in action && (action.report_type == 'xlsx' || action.report_type == 'qweb-html' || action.report_type == 'qweb-pdf' || action.report_type == 'controller')) {
                var report_url = '';
                switch (action.report_type) {
                    case 'xlsx':
                        report_url = '/report/xlsx/' + action.report_name;
                        break;
                    case 'qweb-html':
...
workers to print a pdf version of the reports.'), true);
 ...
 target="_blank">wkhtmltopdf.org</a>'), true);
                ...
                } else if (action.report_type === 'controller' || action.report_type === 'xlsx') {
                    return trigger_download(self.session, response, c, action, options);
                }
            } else {
